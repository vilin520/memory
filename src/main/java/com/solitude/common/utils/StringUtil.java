...
public class StringUtil {
    ...
    /**
     * 日期转换成字符串，返回yyyy-MM-dd HH:mm:ss格式
     *
     * @param date
     * @return str
     */
    ...
    /**
     * 生成非重复订单号，理论上限1毫秒1000个，可扩展 提现CAS开头的TYPE时，第二个参数为用户ID
     *
     * @param
     */
    public static String makeOrderNum(String type, Integer usrId) {
        // 最终生成的订单号
        String finOrderNum = "";
        try {
        ...
        finOrderNum = getOrderStartStr(type, usrId) + finOrderNum;
        return finOrderNum;
    }
     ...
     *
     */
    public static String makeOrderNum(String type, String equNumStr) {
        int equNum = Integer.parseInt(equNumStr);
        // 最终生成的订单号
        String finOrderNum = "";
        try {
            synchronized (lockObj) {
        ...
        finOrderNum = getOrderStartStr(type, equNum) + finOrderNum;
        return finOrderNum;
    }
    ...
    /**
     * 获取订单开头,返回type+参数补足9位数
     *
     * @param
     * @return
     */
    public static String getOrderStartStr(String type, Integer num) {
        String result = type + String.format("%0" + 9 + "d", num);
        return result;
    }
     ...
     * @return
     */
    public static String phoneNumHide(String phoneNum) {
    ...
    public static void main(String[] args) {
        String ss = getOrderStartStr("PAY", 612547);
    ...
    public static boolean checkSignature(Map<String, String> map) {
        String authCode = "taiyixianshan";// 授权码默认写死
        ...
        for (String k : keyArray) {
            if ("sign".equals(k)) {
                logger.info("入参sign：" + map.get(k).toString());
                continue;// _sign不参与签名
            }
            sb.append(k).append("=").append(map.get(k)).append("&");
        }
        sb.append(authCode);
        logger.info("组织签名，入参：" + sb.toString());
        logger.info("组织签名，本地生成的签名结果："+MD5(sb.toString()));
        if (MD5(sb.toString()).equals(sign)) {
            logger.info("组织签名，本地生成的签名结果2："+MD5(sb.toString()));
            return true;
        }
        return false;// 先默认都签名成功
    }
